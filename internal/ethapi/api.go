...
func doCall(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	...
	result, err := core.ApplyMessage(evm, (*core.Message)(msg), gp)
...
func AccessList(ctx context.Context, b Backend, blockNrOrHash rpc.BlockNumberOrHash, args TransactionArgs) (acl types.AccessList, gasUsed uint64, vmErr error, err error) {
	...
	for {
		...
		res, err := core.ApplyMessage(vmenv, (*core.Message)(msg), new(core.GasPool).AddGas(msg.GasLimit))
		if err != nil {
...
func (s *BundleAPI) EstimateGasBundle(ctx context.Context, args EstimateGasBundleArgs) (map[string]interface{}, error) {
	...
	for i, txArgs := range args.Txs {
		...
		// Prepare the hashes
		txContext := core.NewEVMTxContext((*core.Message)(msg))

		// Get EVM Environment
		vmenv := vm.NewEVM(blockContext, txContext, statedb, s.b.ChainConfig(), vm.Config{NoBaseFee: true})

		// Apply state transition
		result, err := core.ApplyMessage(vmenv, (*core.Message)(msg), gp)
		if err != nil {
			return nil, err
		}
